...
DB_CONFIG = {
    ...
    'tabla': 'results_refrigerador_danone_iot'
}
    ...
    def generar_y_subir_csv(self, json_path, csv_path):
        try:
            ...
            with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                writer.writerow(['product_id', 'product_name', 'count', 'timestamp', 'imagen'])
                for nombre_archivo, datos in json_data.items():
                    ...
                    for producto, cantidad in datos['conteo'].items():
                        ...
                        if product_id:
                            writer.writerow([product_id, producto, cantidad, timestamp_proc, nombre_archivo])
                        else:
            ...
            with open(csv_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    try:
                        cursor.execute(f"""
                            INSERT INTO {tabla_completa} (product_id, product_name, count, timestamp, imagen)
                            VALUES (?, ?, ?, ?, ?)
                        """, (
                            int(row['product_id']),
                            row['product_name'],
